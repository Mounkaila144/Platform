...
class FilierController extends AbstractController
{
    ...
    public function new(Request $request, FilierRepository $filierRepository): Response
    {
        $filier = new Filier();
    ...
    public function edit(Request $request, Filier $filier, FilierRepository $filierRepository): Response
    {
        $form = $this->createForm(FilierType::class, $filier);
        $form->handleRequest($request);
    ...
    public function delete(Request $request, Filier $filier, FilierRepository $filierRepository): Response
    {
        if ($this->isCsrfTokenValid('delete'.$filier->getId(), $request->request->get('_token'))) {
            $filierRepository->remove($filier);
        }
